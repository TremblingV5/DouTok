...
func (s *CommentServiceImpl) CommentAction(ctx context.Context, req *comment.DouyinCommentActionRequest) (resp *comment.DouyinCommentActionResponse, err error) {
	if result := misc.CheckCommentActionArgs(req); !result {
		return pack.PackCommentActionResp(int32(misc.ParamsErr.ErrCode), misc.ParamsErr.ErrMsg, nil)
	}
	...
	if req.ActionType == 1 {
		...
		if err != nil {
			return pack.PackCommentActionResp(int32(misc.SystemErr.ErrCode), misc.SystemErr.ErrMsg, nil)
		}
		return pack.PackCommentActionResp(int32(misc.Success.ErrCode), misc.Success.ErrMsg, result)
	} else if req.ActionType == 2 {
		errNo, err := service.RmComment(req.UserId, req.CommentId)
		if err != nil {
			return pack.PackCommentActionResp(int32(errNo.ErrCode), errNo.ErrMsg, nil)
		}
		return pack.PackCommentActionResp(int32(misc.Success.ErrCode), misc.Success.ErrMsg, nil)
	} else {
		return pack.PackCommentActionResp(int32(misc.BindingErr.ErrCode), misc.BindingErr.ErrMsg, nil)
	}
}
