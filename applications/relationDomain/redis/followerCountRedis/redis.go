...
func (c *Client) Get(ctx context.Context, userId int64) (int64, error) {
	ret, err := c.client.HGet(ctx, fmt.Sprintf("%d", userId), constants.FollowerCount)
	if err != nil {
		return 0, err
	}
...
func (c *Client) Set(ctx context.Context, userId, count int64) error {
	err := c.client.HSet(ctx, fmt.Sprintf("%d", userId), constants.FollowerCount, fmt.Sprintf("%d", count))
	if err != nil {
		return err
	}
	return nil
...
func (c *Client) Del(ctx context.Context, userId int64) error {
	_, err := c.client.HDel(ctx, fmt.Sprintf("%d", userId), constants.FollowerCount)
	if err != nil {
		return err
	}
	return nil
