...
type QueryFavoriteCountService struct {
	ctx context.Context
}

func NewQueryFavoriteCountService(ctx context.Context) *QueryFavoriteCountService {
	return &QueryFavoriteCountService{ctx: ctx}
}

func (s *QueryFavoriteCountService) QueryFavoriteCount(video_id []int64) (map[int64]int64, error) {
...
func ReadCountFromCache(videoId int64) (int64, bool, error) {
	data, err := RedisClients[misc.FavTotalCntCache].Get(context.Background(), fmt.Sprint(videoId))
